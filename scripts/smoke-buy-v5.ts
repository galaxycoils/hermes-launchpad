#!/usr/bin/env npx tsx
// SMOKE buy using fee wallet (GkHE2vb8j...) which has 16 SOL and is the keypair.
// Uses canonical ATA address computed with fixed findAta ordering.
...
const CONFIG = {
  ...
  TRADER: 'GkHE2vb8j3PGyjMvCmWJMffiKb2QwVye5TfuUPG1NK5a',  // fee wallet
...
if (!kp.publicKey.equals(TRADER)) {
  console.error('Keypair does not match trader wallet!');
  console.error('Keypair:', kp.publicKey.toBase58());
  console.error('Trader :', TRADER.toBase58());
  process.exit(1);
}

// Canonical ATA using fixed findAta ordering: [owner, mint, TOKEN_PROGRAM_ID]
...
console.log('Trader ATA     :', TRADER_ATA_B58, '(canonical)');
...
console.log(`\nBuy: ${SMOKE_AMOUNT_SOL} SOL → SMOKE`);
...
// Build buy instruction (matches fixed buildTradeIx with canonical ATA)
