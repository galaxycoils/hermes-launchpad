...
try {
  // Use execFileSync with argument array (no shell injection risk — all args are static constants)
  ...
  if (match) {
    ...
    console.log('Could not parse ATA address from output. Check manually.');
    process.exit(1);
  }
} catch (e) {
  ...
  if (msg.includes('Account already exists')) {
    ...
    if (match) {
      ...
      console.log('ATA already exists — check spl-token accounts for address');
      console.log('Next: run smoke-buy-final.ts to buy 0.02 SOL SMOKE');
    }
  } else {
