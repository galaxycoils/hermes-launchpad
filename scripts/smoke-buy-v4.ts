...
// Compute canonical ATA using findProgramAddressSync (no on-curve check)
...
console.log('Canonical ATA   :', CANONICAL_ATA_B58);
...
console.log('Trader SOL:', (traderLamports / LAMPORTS_PER_SOL).toFixed(4));
...
console.log(`\nBuy amount: ${SMOKE_AMOUNT_SOL} SOL = ${lamports} lamports`);
console.log(`Min out   : ${minOut}\n`);

// Create canonical ATA if missing
const ataInfo = await connection.getAccountInfo(canonicalAta);
if (!ataInfo) {
  console.log('Canonical ATA not on-chain — creating using createAssociatedTokenAccountInstruction...');
  const rent = await connection.getMinimumBalanceForRentExemption(165);
  const createAtaIx = createAssociatedTokenAccountInstruction(
    kp.publicKey, canonicalAta, TRADER, MINT, TOKEN_PROGRAM_ID, ATA_PROGRAM_ID,
  );
...
// Build buy instruction using canonical ATA
