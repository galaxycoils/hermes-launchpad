#!/usr/bin/env npx tsx
// SMOKE buy using fee wallet (GkHE2vb8j...) which has 16 SOL.
// Uses the spl-token CLI-created ATA at 59tgQcjN... (canonical address).
...
const CONFIG = {
  ...
  // The trader wallet IS the fee wallet (keypair we have, 16 SOL)
...
// ATA address created by spl-token CLI (canonical ordering)
...
console.log('Trader ATA     :', TRADER_ATA.toBase58(), '(from spl-token CLI)');
console.log('Fee wallet     :', CONFIG.FEE_WALLET);
console.log('Creator wallet :', CONFIG.CREATOR_WALLET);
console.log('Keypair public :', kp.publicKey.toBase58());
...
console.log('Trader SOL balance:', (traderLamports / LAMPORTS_PER_SOL).toFixed(4));

// Verify ATA exists on-chain
const ataInfo = await connection.getAccountInfo(TRADER_ATA);
if (!ataInfo) {
  console.error('ATA not found on-chain at', TRADER_ATA.toBase58());
  console.error('Create it first with: spl-token create-account', CONFIG.MINT, '-C', CONFIG.KEYPAIR.replace('.json', '.yml'), '--fee-payer', CONFIG.KEYPAIR);
  process.exit(1);
}
console.log('ATA on-chain: lamports=' + ataInfo.lamports + ', data length=' + ataInfo.data.length);
console.log('');
