...
import KOHStrip from '../../src/components/KOHStrip'
...
const renderCard = (value: Token) =>
  renderToStaticMarkup(React.createElement(TokenCard, { token: value, onSelect: () => undefined }))
...
describe('WU-05 truthful token presentation', () => {
  ...
  it('renders shared migration threshold math in KOHStrip', () => {
    const html = renderToStaticMarkup(
      React.createElement(KOHStrip, {
        token: token({ onchainMint: 'mint', realSol: MIGRATION_TARGET / 2 }),
        onSelect: () => undefined,
      })
    )
    expect(html).toContain(`${(MIGRATION_TARGET / 2).toFixed(1)} SOL raised`)
