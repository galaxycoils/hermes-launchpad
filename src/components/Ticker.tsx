...
export default function Ticker({ tokenNames, onSelect }: { tokenNames: Record<string, string>; onSelect?: (tokenId: string) => void }) {
  ...
  const items = trades.map((t) => {
    ...
    const ago = Math.max(1, Math.floor((Date.now() / 1000 - t.ts) / 60));
    return { ...t, name, ago };
  });
  ...
  return (
    <div className="relative overflow-hidden border-y border-white/10 bg-black/40 py-1.5">
      ...
      <div
        ...
        style={{ animation: `hermes-ticker ${Math.max(30, strip.length * 4)}s linear infinite` }}
      >
        {strip.map((t, i) => (
          <button
            key={`${t.id}-${i}`}
